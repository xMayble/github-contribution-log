...
Screenshot/logs: isNumber/ directory contains only test_smoke_isNumber.py
...
Plan:
1. Create test_isNumber_numeric_types.py — parametrized tests for int32, int64, double, Decimal128 → true
2. Create test_isNumber_non_numeric_types.py — parametrized tests for string, bool, array, object, ObjectId, Date, etc. → false
3. Create test_isNumber_null_missing.py — null → false, missing field → false

Implement: https://github.com/xMayble/functional-tests/tree/fix-issue-212

Review: Followed CONTRIBUTING.md rules: docstrings on every test function, one assertion per test, execute_command wrappers only. Ran --collect-only to confirm format validator passes before committing.
...
Unit Tests
[x] test_isNumber_numeric_types.py — 48 parametrized tests covering int32, int64, double, Decimal128 as both literals and field references
[x] test_isNumber_non_numeric_types.py — 34 parametrized tests covering string, bool, array, object, ObjectId, Date, Timestamp, Binary, Regex, MinKey, MaxKey, Code
[x] test_isNumber_null_missing.py — 4 tests for null literal, null-valued field, missing field, nested missing path

Integration Tests
[x] All tests run through the aggregation pipeline using execute_expression and execute_expression_with_insert
[x] Original smoke test still collects and passes format validation (regression check)
...
Week 1 Progress
Built all three test files in one session. The main challenge was understanding how the project structures parametrized tests — spent time reading the $and and $toDate operator tests to understand the ExpressionTestCase and BaseTestCase patterns before writing anything. Once the pattern was clear, implementing the tests was straightforward.

Code Changes
Files added:
- documentdb_tests/compatibility/tests/core/operator/expressions/type/isNumber/test_isNumber_numeric_types.py
- documentdb_tests/compatibility/tests/core/operator/expressions/type/isNumber/test_isNumber_non_numeric_types.py
- documentdb_tests/compatibility/tests/core/operator/expressions/type/isNumber/test_isNumber_null_missing.py

Key commit: d8de588 — "Add $isNumber compatibility tests for numeric types, non-numeric types, null, and missing fields"

Branch: https://github.com/xMayble/functional-tests/tree/fix-issue-212

Approach decisions: Used a custom IsNumberTest dataclass extending BaseTestCase with a value field, allowing each test case to drive both a literal test and a field-reference test without duplicating the test data.
